...
_crates = [B1,B2,B3,B4,B5,B6,B7,B8,B9,B10,B11];

//Exclude classes below
...

{
	...
	if (alive (_c select 0)) then {
		if (count (_c select 1) != 0) then {
			{if !(_x in _allVanillaContent) then {_allItems pushBackUnique _x;};} forEach (_c select 1);
		};
		if (count (_c select 2) != 0) then {
			{if !(_x in _allVanillaContent) then {_allBPs pushBackUnique _x;};} forEach (_c select 2);
		};

		{if !(_x in _allVanillaContent) then {_allMags pushBackUnique _x;};} forEach (_c select 3);

		{if !(_x in _allVanillaContent) then {_allWeps pushBackUnique _x;};} forEach (_c select 4);
	};
